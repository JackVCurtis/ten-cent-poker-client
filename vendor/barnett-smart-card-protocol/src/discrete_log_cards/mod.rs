...
use proof_essentials::zkp::fiat_shamir_rng::FiatShamirRng;
use proof_essentials::zkp::absorb::serialize_to_bytes;
...
impl<C: CurveGroup> BarnettSmartProtocol for DLCards<C> {
    ...
    fn prove_key_ownership<B: CanonicalSerialize, R: Rng>(
        ...
        let mut seed = serialize_to_bytes(KEY_OWN_RNG_SEED);
        seed.extend_from_slice(&serialize_to_bytes(player_public_info));
        let mut fs_rng = FiatShamirRng::<Blake2s256>::from_seed(&seed);
    ...
    fn mask<R: Rng>(
        ...
        let negative_original = original_card.0.mul_bigint(minus_one.into_bigint());
        ...
        let mut fs_rng = FiatShamirRng::<Blake2s256>::from_seed(&serialize_to_bytes(MASKING_RNG_SEED));
    ...
    fn verify_mask(
        ...
        // Map to Chaum-Pedersen statement
        let minus_one = -Self::Scalar::one();
        let negative_original = card.0.mul_bigint(minus_one.into_bigint());
        let statement_cipher = (masked_card.1 + negative_original).into_affine();
        let cp_statement =
            chaum_pedersen_dl_equality::Statement::new(&masked_card.0, &statement_cipher);
    ...
    fn remask<R: Rng>(
        ...
        let mut fs_rng = FiatShamirRng::<Blake2s256>::from_seed(&serialize_to_bytes(REMASKING_RNG_SEED));
        let proof = chaum_pedersen_dl_equality::DLEquality::prove(
            rng,
            &cp_parameters,
            &cp_statement,
    ...
    fn compute_reveal_token<R: Rng>(
        ...
        let reveal_token: RevealToken<C> =
            el_gamal::Plaintext(masked_card.0.mul_bigint(sk.into_bigint()).into_affine());
        ...
        let mut fs_rng = FiatShamirRng::<Blake2s256>::from_seed(&serialize_to_bytes(REVEAL_RNG_SEED));
        let proof = chaum_pedersen_dl_equality::DLEquality::prove(
            rng,
            &cp_parameters,
            &cp_statement,
    ...
    fn shuffle_and_remask<R: Rng>(
        ...
        let mut fs_rng = FiatShamirRng::<Blake2s256>::from_seed(&serialize_to_bytes(SHUFFLE_RNG_SEED));

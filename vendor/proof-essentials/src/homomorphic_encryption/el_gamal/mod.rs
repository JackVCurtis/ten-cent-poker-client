...
// `ScalarField` lives on `PrimeGroup` (a supertrait of `CurveGroup`) in 0.6.
...
// NOTE (0.3 -> 0.6): the manual `ToBytes` impl for `Ciphertext` is dropped;
// `ToBytes` no longer exists in arkworks 0.6. Fiat-Shamir absorption is now done
// via the derived `CanonicalSerialize` (serialize_compressed).
    ...
    C: CurveGroup,
{
    ...
    fn keygen<R: Rng>(
        ...
        // 0.3 `affine.mul(scalar) -> projective`; 0.6 uses `mul_bigint(into_bigint())`.
        let public_key = pp.generator.mul_bigint(secret_key.into_bigint()).into_affine();
    ...
    fn encrypt(
        ...
        let c1 = pp.generator.mul_bigint(r.into_bigint()).into_affine();
    ...
    fn decrypt(
        ...
        // compute s = secret_key * c1  (projective)
        let s = c1.mul_bigint(sk.into_bigint());

        // compute message = c2 - s   (do the subtraction in the group)
        let m = (c2.into_group() - s).into_affine();

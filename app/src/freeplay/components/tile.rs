...
use eframe::egui::{
    self, pos2, Align2, Color32, CornerRadius, Layout, Rect, Sense, Stroke, StrokeKind, Ui,
    UiBuilder, Vec2,
};
...
pub struct TileResponse {
    ...
    pub act: Option<Act>,
    /// Requested change (chips, signed) to the bet/raise sizing target, from the action bar stepper.
    pub size_delta: i64,
...
pub fn render(
    ...
    let Some(table) = state.table(id) else {
        return resp;
    };
    ...
    let strip_rect = Rect::from_min_max(pos2(rect.min.x, rect.max.y - strip_h), rect.max);
    ...
    let bar = action_bar::render(ui, strip_rect, table, pulse);
    resp.act = bar.act;
    resp.size_delta = bar.size_delta;
...
fn right_cluster(ui: &mut Ui, rect: Rect, table: &Table, focused: bool) -> bool {
    ...
    let menu_resp = ui.interact(
        menu_hit,
        ui.id().with(("tile_menu_btn", table.id)),
        Sense::click(),
    );
    let menu_color = if menu_resp.hovered() {
        Palette::TEXT_SECONDARY
    } else {
        Color32::from_rgb(0x56, 0x5a, 0x62)
    };
    // Three dots right-aligned to `x`, 5px apart, vertically centred.
    for k in 0..3 {
        ui.painter()
            .circle_filled(pos2(x - 2.0 - k as f32 * 5.0, cy), 1.4, menu_color);
    }
    ...
    if focused {
        ...
        let tag = Rect::from_min_size(
            pos2(x - lw.x - pad.x * 2.0, cy - (lw.y + pad.y * 2.0) / 2.0),
            lw + pad * 2.0,
        );
        theme::fill_rect(ui, tag, rad::BADGE, fill, Stroke::NONE);
        ui.painter()
            .text(tag.center(), Align2::CENTER_CENTER, label, font, text);
...
fn right_cluster_width(ui: &Ui, table: &Table, focused: bool) -> f32 {
    ...
    let blinds_w = painter
        .layout_no_wrap(
            table.blinds.clone(),
            theme::mono_font(10.0, Weight::Regular),
            Palette::TEXT_MUTED_DIM,
        )
        .size()
        .x;
...
fn badge_width(ui: &Ui, label: &str, mono: bool) -> f32 {
    ...
    ui.painter()
        .layout_no_wrap(label.to_string(), font, Palette::TEXT_SECONDARY)
        .size()
        .x
        + 12.0
}
